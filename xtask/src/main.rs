...
fn run_qemu_aarch64() -> Result<()> {
    ...
    let res = (|| -> Result<()> {
        ...
        // B-HAL.5.1: tick-count line printed by EL0 — proves the lower-EL
        // IRQ vector (slot 9) is wired and timer IRQs taken *at EL0* go
        // through the same full-frame irq_stub used at EL1, round-tripping
        // back to the user routine cleanly.
        wait_for_output(&buf, "ticks=", 30)?;
        wait_for_output(&buf, "[el0] EL0 + SVC roundtrip: ok (IRQs at EL0 too)", 30)?;
        // B-HAL.5.2: a separately-compiled aarch64 user ELF (the
        // `user-aarch64-hello/` standalone crate, baked into the kernel image
        // by build.rs) is parsed + loaded by the kernel's minimal ELF reader,
        // erets to its entry at EL0, runs to completion through the same SVC
        ...
        // B-HAL.5.4a: write a known pattern to sector 1, read it back,
        // verify match — each transfer wrapped in a fresh `BlockRequest`
        // FSM (the same pure Frame system the x86 path uses). Proves the
        // Frame layer composes with virtio-mmio: the FSM hits $Complete
        // only when the device returned status 0, and the bytes round-trip
        // exactly.
        wait_for_output(
            &buf,
            "[vio-mmio] sector 1 write+read+verify via BlockRequest FSM: ok",
            30,
        )?;
        ...
        // Runnable thread, and `eret` switches into it — the same pattern x86
        // sched.rs uses (full-frame save + round-robin). Both workers
        // exiting → the Frame Scheduler reports $Idle → the boot context
        // returns.
        wait_for_output(&buf, "[preempt] starting two non-yielding threads", 30)?;
        wait_for_output(
            &buf,
