...
**`Scheduler`** — the run/halt *mode* (`$Idle`/`$Active`), deliberately minimal (see [`systems/scheduler.md`](systems/scheduler.md) for why the speculative `$PickingNext`/`$ContextSwitching` graph was dropped). The actual register/stack/address-space mechanics are native (`sched.rs`): a preemptive round-robin over kernel threads **and user processes**. *As of B3 Step 5a* a user process is a scheduled entity with its own address space (PML4) and ring-0 kernel stack; the timer-ISR switch saves/restores the full trap frame and switches **CR3 + `TSS.RSP0`** per process, so user processes are preemptible in ring 3 and run concurrently. The Frame `Scheduler` rides on top, counting runnable entities to drive the boot context's `$Idle` halt decision.
