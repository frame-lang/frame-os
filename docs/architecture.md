...
**`Task`** (B1) / **`Process`** (B4) — per-task or per-process lifecycle. The textbook state graph: `$Created → $Ready → $Running → $Blocked → $Zombie → $Reaped`. Each instance is one task or process. The kernel holds a fixed-size array of these. State-dependent dispatch makes `kill()` mean different things in different states: from `$Running` it transitions to `$Zombie`; from `$Blocked` it transitions to `$Zombie` and frees the wait queue entry; from `$Zombie` it's a no-op.

**`ProcessTable`** (B4) — slot management for the process array. States per slot: `$Free → $Reserved → $Active → $ZombieAwaitingReap → $Free`. The `$Reserved` state exists for in-progress `fork()` operations that might fail partway through; the slot is reserved but not committed.
...
**`ElfLoader`** (B4) — parses ELF bytes and produces a process image. States represent loading phases: `$ReadingHeader → $ValidatingHeader → $MappingSegments → $BuildingStack → $Done`, with `$Failed` as a sink that cleans up partial work. The HSM lets the loader funnel all failure paths through a single `$Failed` state that handles partial cleanup, avoiding a `Result<>` ladder that would obscure the load sequence.
