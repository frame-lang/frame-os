...
- **2026-05-27** — M3b.2/.3 (H↔B parity): the Shell FSM runs in **ring 3**. `frame/shell.frs` now also compiles for `x86_64-unknown-none` — `ish` includes it in a local `shell_fsm` module and supplies `IshShellEnv` (the `ShellEnv` seam over syscalls: print via `write`, ish's builtin set → `CommandKind`, `run_external`/`run_pipeline`/fork/exec/wait + the `IshJobs` table; the synchronous fork+wait splits across `spawn_foreground`/`fg` + `wait_foreground`). `ish`'s `_start` drives the *same* Shell control-flow FSM the hosted shell uses; its hand-written `run_line` + `fg_job` are retired. Validated: `console-test` green end-to-end (full S1–S10 — builtins, redirection, pipes, background jobs, signals, job-control suspend/resume, on-device tcc), now Shell-FSM-driven; user-crate clippy + fmt clean. **The same `Shell` state machine now drives a shell on Linux *and* bare metal** — the H↔B parity headline.
- **2026-05-27** — M3b.1 (H↔B parity): `ShellEnv` seam. Every target-specific operation the Shell FSM performed — `print_prompt`/`print_goodbye`/`tick`, `classify`, `run_builtin`, `run_foreground_redirected`, `spawn_background`, `spawn_foreground`, `fg`, `run_pipeline`, `wait_foreground`, `println` — moved behind the `ShellEnv` trait (declared in the shell.frs prolog alongside the `CommandKind` routing enum). Domain dropped `current_builtin: Builtin`, `last_foreground_id`, `cwd: PathBuf`, and `job_control: JobControl` in favour of `env: Box<dyn ShellEnv> = default_env()` (+ `current_words`). The hosted `StdShellEnv` (shell/src/shell_env.rs) wraps the existing classify/execute/exec + JobControl + cwd, preserving behavior exactly. **State graph unchanged** (snapshot byte-stable) — the FSM is now environment-agnostic so the *same* state machine can compile for ring 3 (M3b.2/.3). Same FSM-owns-logic / native-owns-mechanism split as the M2 process backend, applied to the whole shell. All hosted tests green.
