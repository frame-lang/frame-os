...
| B3-7 | `fork` + `exec` spawns a child that runs independently; parent reaps via wait | QEMU smoke — **`fork` (Step 5b, `fork_concurrency_b3`) + `exec` (Step 5c, `exec_b3`: child execs `hello`) done**; `wait`/reap (Step 5d) pending |
...
- **Remaining:** Step 5d — `wait` + signals (SIGCHLD/SIGKILL/SIGSEGV) + child reaping & resource teardown. That closes B3.
