...
- **Step 3a/3b (`UsbEnumeration` Frame system — through addressing) — done.** The device-enumeration lifecycle: `$Powered → $SlotEnabled → $AddressAssigned → …`, with `fail()` funneled to `$Failed` from any active stage through an `$Enumerating` parent (`=> $^`). Each state's enter handler issues the **next xHCI command, non-blocking** (Enable Slot in `$Powered`, Address Device in `$SlotEnabled`), and `xhci::run_enumeration()` dequeues the Command Completion Events off the event ring and dispatches the milestone event (`slot_enabled(slot)`/`addressed()`) — the same "enter kicks the async step, completion event advances the FSM" shape as TCP, now over xHCI command/event rings. The assigned **slot id threads through the FSM domain**. This drove the meatiest native xHCI work: the **command ring** (TRB enqueue + cycle bit + Link-TRB wrap), the **event ring** (cycle-state dequeue + ERDP), **doorbells**, and the **Input Context** (Input Control + Slot + EP0 contexts, `ctx_64`-aware, speed/MPS/port) + the output device context registered in the DCBAA. Enumerates the real qemu-xhci usb-kbd to a USB address — serial: `[usb] slot 1 enabled` → `[usb] device addressed (slot 1)`. Host-tested (snapshot + 6 behavioral incl. the fail funnel) and validated by `usb_enumerates_b6` (**31/31** QEMU). Per-system doc + SVG committed. **Remaining (Step 3c):** EP0 control transfers (GET_DESCRIPTOR + SET_CONFIGURATION) → `$Configured`, then a transfer (B6-3).
