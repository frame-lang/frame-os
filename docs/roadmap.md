...
| B7-3 | Lock ordering documented; no deadlock under a stress workload | **Started (Step 2)** — lock ordering documented in `spin.rs` (all leaves so far); the 4-core × 50 000 counter hammer (`smp_concurrent_b7`) is a stress workload that completes without deadlock or lost updates. Revisit as nested locks are added |
