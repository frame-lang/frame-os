...
| B5-5 | ARP resolution and IP reassembly correct | **Mostly done** — ARP resolution done (`ArpResolver`, `arp_resolves_gateway_b5`); the kernel also *answers* inbound ARP who-has-10.0.2.15 over TAP (`qemu-tap`, the precondition for the inbound ping). IP reassembly still not exercised (slirp/TAP deliver unfragmented) — open |
...
**B5 status: substantially complete.** The core (NIC + ARP + IPv4/ICMP + UDP + the full TCP exchange) is done and validated, and the kernel answers a real inbound `ping` over TAP (B5-3, Step 5). **Open:** IP reassembly (B5-5 — slirp/TAP deliver unfragmented, so it isn't exercised yet). **Deferred to the framec release:** crates.io CI (B5-7). The "stress-test Frame" findings are recorded in [`frame_assessment.md`](frame_assessment.md).
