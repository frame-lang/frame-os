...
| # | Exit criterion | Validating test(s) |
|---|---|---|
| B3-1 | `Process`, `ProcessTable`, `SyscallDispatcher`, `ElfLoader` state graphs match committed designs | Snapshots (`kernel-tests`) — **`SyscallDispatcher` done at Step 2; `Process`/`ProcessTable` done at Step 3** (`process_state_graph_snapshot`, `process_table_state_graph_snapshot`); `ElfLoader` pending |
...
- **Remaining:** `ElfLoader` + a real user ELF (Step 4), `fork`/`exec` + signals (Step 5).
