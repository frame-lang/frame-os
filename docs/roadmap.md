...
| # | Exit criterion | Validating test(s) |
|---|---|---|
| B4-1 | `BlockRequest`, `OpenFile`, `Mount` state graphs match committed designs | Snapshots (`kernel-tests`) — **`BlockRequest` (Step 1) + `Mount` (Step 2) done**; `OpenFile` pending (Step 3) |
| B4-2 | I/O request + file + mount lifecycles correct | Behavioral tests (host) — **`BlockRequest` (6, Step 1) + `Mount` (5, Step 2) done**; `OpenFile` pending |
...
- **Remaining:** Step 3 — VFS + `OpenFile` + path lookup / directories. Step 4 — the userspace shell.
