...
| B3-7 | `fork` + `exec` spawns a child that runs independently; parent reaps via wait | QEMU smoke — **`fork` done at Step 5b** (`fork_concurrency_b3`: child runs concurrently in its own address space); `exec` (Step 5c) + `wait`/reap (Step 5d) pending |
...
- **Remaining:** Step 5c — `exec` (replace the current image with a freshly loaded ELF). Step 5d — `wait` + signals (SIGCHLD/SIGKILL/SIGSEGV) + resource teardown.
