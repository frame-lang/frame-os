...
| B4-6 | `Shell`/`Parser` per-system docs gain a "userspace action implementations" note; same `.frs` builds for host and userspace | Review; build check — **pending (Step 4b)**; needs an allocator in the `user/` crate |
...
- **Remaining:** Step 4b — reuse the `Shell`/`Parser` `.frs` in ring 3 (the "same source builds for host and userspace" demonstration, B4-6): add an allocator to the `user/` crate, compile `parser.frs`/`shell.frs` for userspace, and drive them from a ring-3 program. The B4 headline. Then Step 4c/B4-7 — diagrams + docs + CI gates.
