...
| B6-3 | A QEMU virtual USB device (e.g. keyboard or mass-storage) enumerates to `$Configured` and completes a transfer | **Enumeration half done** — the qemu-xhci `usb-kbd` enumerates to `$Configured` (`usb_enumerates_b6`: Enable Slot → Address Device → GET_DESCRIPTOR → SET_CONFIGURATION). The *transfer* half (a HID interrupt-IN report) lands with `UsbTransfer` (Step 4). |
