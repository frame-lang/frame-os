// Frame OS user program "rm" (S3) — delete files. `rm <path>...` unlinks each
// argument via the unlink syscall (#17); paths resolve against the shell's cwd
// (inherited on fork). Disk-only: the shell runs it as `/bin/rm`.
//
// Syscall ABI: 1 = exit, 12 = write(fd, buf, len), 17 = unlink(path, len).
...
#[no_mangle]
extern "C" fn rm_main(sp: *const u64) -> ! {
    let argc = unsafe { *sp };
    if argc < 2 {
        write(1, b"rm: usage: rm <path>...\n");
        exit(1);
    }
    let mut status = 0u64;
    let mut i = 1u64;
