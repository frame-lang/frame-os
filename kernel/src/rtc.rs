// kernel/src/rtc.rs
//
// CMOS real-time clock (Motorola MC146818-compatible) → Unix epoch seconds.
//
// The on-device toolchain's libc `time()`/`gettimeofday()`/`localtime()` read
// this so a tcc-compiled program's `__DATE__`/`__TIME__` (which tcc itself
// computes via `time()` + `localtime()` while preprocessing) and any user
// `time()` call reflect real wall-clock time, not a fixed 1970/2026 stub.
//
// Mechanism: the RTC is read through the CMOS index/data port pair (0x70/0x71).
// Register 0x0A's bit 7 is "update in progress" (UIP) — the chip is mid-tick and
// the time registers are unstable. Register 0x0B says whether the values are BCD
// or binary (bit 2) and whether hours are 24-hour (bit 1). We wait out UIP, read
// all fields twice until two reads agree (so we never latch a half-updated
// value), decode BCD/12-hour as the status byte dictates, then fold the
// broken-down civil time into epoch seconds. The RTC is treated as UTC — the
// same convention QEMU's `-rtc base=<timestamp>` presents.
...
/// Current wall-clock time as Unix epoch seconds (UTC). Reads the RTC twice and
/// retries until two consecutive reads agree, so a mid-update tick is never
/// latched.
pub fn epoch_secs() -> u64 {
    ...
    // In 12-hour mode the PM flag is bit 7 of the *raw* hour byte (set before BCD
    // decoding), so strip it before converting.
