...
global_asm!(
    ...
    "  add  sp, sp, #272",
    "  eret",
    // The SVC stub — full-frame save (B-HAL.5.0). Structurally identical to
    // irq_stub: the saved frame is in the same TrapFrame layout (x0..x30 +
    // ELR_EL1 + SPSR_EL1 + pad, 272 B), and the Rust handler is called with
    // the saved-frame SP + returns an SP to restore on. The difference is
    // *which* Rust function — `rust_svc_handler` reads ESR_EL1, dispatches
    // the SVC number from x8, may modify the frame's x0 (the syscall return
    // value), or — for the exit syscall — replace ELR_EL1 + SPSR_EL1 in the
    // frame to redirect the `eret` back into the kernel context that
    // originally `enter_el0`'d into the user routine.
    ".global svc_stub",
    "svc_stub:",
    "  sub  sp, sp, #272",
    "  stp  x0,  x1,  [sp, #0]",
